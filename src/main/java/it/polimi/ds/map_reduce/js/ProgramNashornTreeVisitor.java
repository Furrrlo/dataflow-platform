...
public final class ProgramNashornTreeVisitor extends ThrowingNashornTreeVisitor<Program, ProgramNashornTreeVisitor.Ctx> {
    ...
    public static Program parse(String src, CompilationUnitTree cut) {
    ...
    protected record Ctx(String sourceCode,
        ...
        public Ctx transitionState() {
            return new Ctx(sourceCode, lineMap, state.next(), src, partitions, ops);
        }

        public Ctx withPartitions(int partitions) {
            return new Ctx(sourceCode, lineMap, state, src, partitions, ops);
        }

        public Ctx withSrc(Src src) {
            return new Ctx(sourceCode, lineMap, state, src, partitions, ops);
        }
    }
    ...
    private Program visitSrcFnCall(FunctionCallTree node, Ctx ctx) {
        ...
        var kind = Arrays.stream(Src.Kind.values())
        ...
        Src src = switch (kind) {
            case LINES -> new LinesSrc(fileName);
            case CSV -> {
                if(node.getArguments().size() == 1)
                    yield new CsvSrc(fileName);
                ...
                yield new CsvSrc(fileName, delimiter);
            }
        };
